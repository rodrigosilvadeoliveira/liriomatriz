...
// Verificar se existe uma escala salva para este usuário
// No início do arquivo principal, substitua a consulta atual por:
$escalaSalva = null;
$sql = "SELECT * FROM escalas_salvas WHERE id = 1 LIMIT 1";
...
function addColumn() {
  ...
  function autoFillTable() {
    ...
    // Selecionar todos os selects na tabela
    const selects = document.querySelectorAll('#escalaTable select');
    
    selects.forEach(select => {
      if (select.options.length > 1) {
        // Selecionar uma opção aleatória (exceto a primeira que é vazia)
